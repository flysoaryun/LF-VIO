...
bool ProjectionTdFactor::Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
{
    ...
    double i_theta = atan2(pts_i_td(2), sqrt(pts_i_td(0) * pts_i_td(0) + pts_i_td(1) * pts_i_td(1))) + M_PI / 2;
    double j_theta = atan2(pts_j_td(2), sqrt(pts_j_td(0) * pts_j_td(0) + pts_j_td(1) * pts_j_td(1))) + M_PI / 2;

    Eigen::Matrix2d sqrt_info_new = sqrt_info;
    ...
    residual = sqrt_info_new * residual;
