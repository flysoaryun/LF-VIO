								...
								vector<SFMFeature> &sfm_f)
{
	...
	for (int j = 0; j < feature_num; j++)
	{
		...
		for (int k = 0; k < (int)sfm_f[j].observation.size(); k++)
		{
			if (sfm_f[j].observation[k].first == i)
			{
				...
				Vector3d world_pts(sfm_f[j].position[0], sfm_f[j].position[1], sfm_f[j].position[2]);
	...
	pnp1.set_maximum_number_of_correspondences(featureNums);
	for (auto num = 0; num < pts_3_vector.size(); num++)
	{
		pnp1.add_correspondence(pts_3_vector[num], pts_2_vector[num]);
	}
